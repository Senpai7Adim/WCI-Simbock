    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center relative z-50">
          ...
          <div className="hidden lg:flex items-center gap-1 md:gap-6 text-sm font-medium text-stone-500">
            {links.map(link => {
              ...
              return (
                <Link 
                  key={link.path} 
                  to={link.path} 
                  className={`flex flex-col items-center justify-center min-w-[80px] min-h-[52px] hover:text-nobel-gold transition-colors ${isActive ? 'text-nobel-gold border-b-2 border-nobel-gold' : ''}`}
                >
            ...
            <div className="flex flex-col items-center justify-center min-w-[80px] min-h-[52px] border-l border-stone-200 pl-6 ml-2">
              {user ? (
                <button onClick={logout} className="flex flex-col items-center hover:text-nobel-gold transition-colors cursor-pointer">
                ...
                <button onClick={login} className="flex flex-col items-center hover:text-nobel-gold transition-colors cursor-pointer">
          ...
          <div className="lg:hidden flex items-center gap-4">
            <button className="text-stone-600 p-2 hover:text-nobel-gold transition-colors" onClick={() => setMenuOpen(!menuOpen)}>
      ...
      <div className={`fixed top-0 left-0 w-full h-[100dvh] z-40 bg-white flex flex-col pt-16 pb-12 overflow-y-auto transition-transform duration-300 ease-in-out lg:hidden ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col px-4 py-2">
          {links.map(link => {
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-4 py-4 border-b border-stone-100 ${isActive ? 'text-nobel-gold font-semibold' : 'text-stone-600'}`}
              >
                {link.icon}
          ...
          <div className="py-6">
            {user ? (
              <button onClick={() => { logout(); setMenuOpen(false); }} className="w-full py-3 bg-white border border-stone-400 text-stone-600 rounded-full font-semibold hover:bg-stone-50 transition-colors">
                Sign Out
              </button>
            ) : (
              <button onClick={() => { login(); setMenuOpen(false); }} className="w-full py-3 bg-nobel-gold text-white rounded-full font-semibold hover:bg-red-700 transition-colors">
                Sign In
              </button>
            )}
          </div>
